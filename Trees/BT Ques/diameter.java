...
public class diameter {
    public static class Node {
      ...
      public static class Pair1{
        int ht;
        int dia;
        Pair1(int ht, int dia){
            this.ht=ht;
            this.dia=dia;
        }
      }
      public static Pair1 diameter(Node node) {
        // write your code here
        if(node==null){
            return new Pair1(-1,-1);
        }

        Pair1 lpair=diameter(node.left);
        Pair1 rpair=diameter(node.right);

        int myDia=lpair.ht+rpair.ht+2;
        int myht=Math.max(lpair.ht,rpair.ht)+1;
        int oDia=Math.max(myDia,Math.max(lpair.dia,rpair.dia));

        return new Pair1(myht,oDia);
      }
